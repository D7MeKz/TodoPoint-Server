...
func (mq *MemberQuery) QueryPointInfo() *PointInfoQuery {
	query := (&PointInfoClient{config: mq.config}).Query()
...
func (mq *MemberQuery) Clone() *MemberQuery {
	...
	return &MemberQuery{
		...
		withPointInfo: mq.withPointInfo.Clone(),
...
func (mq *MemberQuery) WithPointInfo(opts ...func(*PointInfoQuery)) *MemberQuery {
	query := (&PointInfoClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withPointInfo = query
	return mq
}
...
func (mq *MemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Member, error) {
	...
	if query := mq.withPointInfo; query != nil {
		if err := mq.loadPointInfo(ctx, query, nodes,
			func(n *Member) { n.Edges.PointInfo = []*PointInfo{} },
			func(n *Member, e *PointInfo) { n.Edges.PointInfo = append(n.Edges.PointInfo, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withTasks; query != nil {
		if err := mq.loadTasks(ctx, query, nodes,
			func(n *Member) { n.Edges.Tasks = []*Task{} },
			func(n *Member, e *Task) { n.Edges.Tasks = append(n.Edges.Tasks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MemberQuery) loadPointInfo(ctx context.Context, query *PointInfoQuery, nodes []*Member, init func(*Member), assign func(*Member, *PointInfo)) error {
	...
	query.withFKs = true
	query.Where(predicate.PointInfo(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(member.PointInfoColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.member_point_info
		if fk == nil {
			return fmt.Errorf(`foreign-key "member_point_info" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "member_point_info" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (mq *MemberQuery) loadTasks(ctx context.Context, query *TaskQuery, nodes []*Member, init func(*Member), assign func(*Member, *Task)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Member)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Task(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(member.TasksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.member_tasks
		if fk == nil {
			return fmt.Errorf(`foreign-key "member_tasks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "member_tasks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

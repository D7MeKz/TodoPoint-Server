...
func (tq *TaskQuery) QuerySubtask() *SubTaskQuery {
	query := (&SubTaskClient{config: tq.config}).Query()
...
func (tq *TaskQuery) WithSubtask(opts ...func(*SubTaskQuery)) *TaskQuery {
	query := (&SubTaskClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withSubtask = query
	return tq
}

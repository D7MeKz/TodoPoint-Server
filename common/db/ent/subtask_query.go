...
func (stq *SubTaskQuery) QueryPoint() *PointQuery {
	query := (&PointClient{config: stq.config}).Query()
...
func (stq *SubTaskQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (stq *SubTaskQuery) FirstIDX(ctx context.Context) int {
...
func (stq *SubTaskQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (stq *SubTaskQuery) OnlyIDX(ctx context.Context) int {
...
func (stq *SubTaskQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (stq *SubTaskQuery) IDsX(ctx context.Context) []int {
	ids, err := stq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (stq *SubTaskQuery) WithPoint(opts ...func(*PointQuery)) *SubTaskQuery {
	query := (&PointClient{config: stq.config}).Query()
...
func (stq *SubTaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SubTask, error) {
	...
	if query := stq.withPoint; query != nil {
		if err := stq.loadPoint(ctx, query, nodes, nil,
			func(n *SubTask, e *Point) { n.Edges.Point = e }); err != nil {
			return nil, err
		}
	}
...
func (stq *SubTaskQuery) loadPoint(ctx context.Context, query *PointQuery, nodes []*SubTask, init func(*SubTask), assign func(*SubTask, *Point)) error {
	...
	query.Where(predicate.Point(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subtask.PointColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.sub_task_point
		if fk == nil {
			return fmt.Errorf(`foreign-key "sub_task_point" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "sub_task_point" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

...
func (mq *MemberQuery) loadTasks(ctx context.Context, query *TaskQuery, nodes []*Member, init func(*Member), assign func(*Member, *Task)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(member.TasksTable)
		s.Join(joinT).On(s.C(task.FieldID), joinT.C(member.TasksPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(member.TasksPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(member.TasksPrimaryKey[0]))

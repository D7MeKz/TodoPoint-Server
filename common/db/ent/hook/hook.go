...
// The PointInfoFunc type is an adapter to allow the use of ordinary
// function as PointInfo mutator.
type PointInfoFunc func(context.Context, *ent.PointInfoMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PointInfoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PointInfoMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PointInfoMutation", m)
}

...
func (mc *MemberCreate) AddPointInfoIDs(ids ...int) *MemberCreate {
	mc.mutation.AddPointInfoIDs(ids...)
	return mc
}

// AddPointInfo adds the "point_info" edges to the PointInfo entity.
func (mc *MemberCreate) AddPointInfo(p ...*PointInfo) *MemberCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return mc.AddPointInfoIDs(ids...)
}

// AddTaskIDs adds the "tasks" edge to the Task entity by IDs.
func (mc *MemberCreate) AddTaskIDs(ids ...int) *MemberCreate {
...
func (mc *MemberCreate) createSpec() (*Member, *sqlgraph.CreateSpec) {
	...
	if nodes := mc.mutation.PointInfoIDs(); len(nodes) > 0 {
